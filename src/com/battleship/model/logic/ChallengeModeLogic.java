...
public class ChallengeModeLogic extends GameLogic {
    ...
    public ChallengeModeLogic(Player player, ChallengeBoardLoader loader) throws IOException {
        ...
        attackInventory.addAttack(AttackType.CROSS, loader.getCrossCount() - attackInventory.getAttackCount(AttackType.CROSS));
        attackInventory.addAttack(AttackType.RANDOM, loader.getRandomCount() - attackInventory.getAttackCount(AttackType.RANDOM));
        attackInventory.addAttack(AttackType.DIAMOND, loader.getDiamondCount() - attackInventory.getAttackCount(AttackType.DIAMOND));
    ...
    /**
     * Thực hiện tấn công tại vị trí (x, y) với loại đạn type.
     * Trả về danh sách các node bị bắn trúng (để View cập nhật hiệu ứng).
     */
    ...
    // --- GETTER ---
    public int getShotsUsed() { return shotsUsed; }
    public int getShotsLeft() { return maxShots - shotsUsed; }
    public int getAttackCount(AttackType type) { return attackInventory.getAttackCount(type); }
